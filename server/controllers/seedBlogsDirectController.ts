...
export async function seedBlogDirectly(req: Request, res: Response) {
  try {
    // Önce mevcut blog yazılarını temizleyelim
    ...
    // Daha fazla blog eklemek için basit içerik oluşturma fonksiyonu
    function generateMoreBlogs(count) {
      const topics = [
        ...
        "Kalıcı Sonuçlar İçin İpuçları: Saç Ekimi Sonrası Yaşam",
        "Saç Ekiminin Psikolojik Etkileri: Özgüven Yolculuğu",
        "Gürcistan'da Saç Ekimi: MyHair Clinic Farkı",
      ];
      ...
      // En fazla topics.length kadar blog ekleyebiliriz
      const numToAdd = Math.min(count, topics.length);
      ...
      for (let i = 0; i < numToAdd; i++) {
        ...
        // Blog içeriği oluştur (basitleştirilmiş)
        const content = `<h2>${topic}</h2><p>Bu konuda detaylı bilgi sahibi olmak, doğru tedavi seçiminde kritik öneme sahiptir. MyHair Clinic uzmanları olarak sizlere en güncel ve bilimsel bilgileri sunuyoruz.</p><h2>Nelere Dikkat Edilmeli?</h2><p>Bu işlem için uygun adayların belirlenmesi, doğru planlama ve sonraki bakım süreci başarılı sonuçlar elde etmek için çok önemlidir.</p><h2>Uzman Görüşü</h2><p>Kliniğimizin deneyimli doktorları, her hasta için kişiselleştirilmiş tedavi planları oluşturarak en iyi sonuçları elde etmeyi hedeflemektedir.</p><h2>Sık Sorulan Sorular</h2><ul><li><strong>İşlem ne kadar sürer?</strong> Tipik olarak 4-8 saat arasında değişebilir.</li><li><strong>İyileşme süreci nasıldır?</strong> İlk birkaç gün içinde hafif şişlik ve kızarıklık olabilir, ancak genellikle 7-10 gün içinde günlük aktivitelere dönülebilir.</li><li><strong>Sonuçlar ne zaman görülür?</strong> İlk sonuçlar 3-4 ay içinde görülmeye başlar, tam sonuçlar 12-18 ay içinde ortaya çıkar.</li></ul>`;
        ...
        additionalBlogs.push({
          ...
          titleRU: topic,
          titleKA: topic,
          summaryTR: `${topic} hakkında kapsamlı bilgiler ve öneriler. MyHair Clinic uzmanlarından tavsiyeler.`,
          summaryEN: `Comprehensive information and recommendations about ${topic}. Advice from MyHair Clinic experts.`,
          summaryRU: `Исчерпывающая информация и рекомендации о ${topic}. Советы экспертов MyHair Clinic.`,
          summaryKA: `ყოვლისმომცველი ინფორმაცია და რეკომენდაციები ${topic} შესახებ. რჩევები MyHair Clinic-ის ექსპერტებისგან.`,
          contentTR: content,
          contentEN: content,
          contentRU: content,
          contentKA: content,
          imageUrl: `/images/blog/${category}-${i % 5 + 1}.jpg`,
          category,
          tags: "hair transplant,MyHair Clinic,expert advice,treatment options",
          author: author.name,
          authorTitle: author.title,
          authorAvatar: author.avatar,
          readingTime: 5 + (i % 4),
          isFeatured: i < 3,
          featuredOrder: i < 3 ? i + 4 : 0,
          ...
          metaDescriptionTR: `${topic} hakkında kapsamlı rehber ve uzman görüşleri. MyHair Clinic öncülüğünde en güncel bilgiler.`,
          metaDescriptionEN: `Comprehensive guide and expert opinions about ${topic}. The most up-to-date information led by MyHair Clinic.`,
          metaDescriptionRU: `Исчерпывающее руководство и мнения экспертов о ${topic}. Самая актуальная информация от MyHair Clinic.`,
          metaDescriptionKA: `ყოვლისმომცველი გზამკვლევი და ექსპერტთა მოსაზრებები ${topic} შესახებ. MyHair Clinic-ის მიერ წარმოდგენილი უახლესი ინფორმაცია.`,
          createdAt: new Date(),
          updatedAt: new Date(),
          isPublished: true
        });
    ...
    // Ana blog yazılarına 10 tane daha ekleyelim
    const allBlogPosts = [...uniqueBlogPosts, ...generateMoreBlogs(10)];
    ...
    console.log(`Toplam ${allBlogPosts.length} blog yazısı başarıyla eklendi.`);
    return res.status(200).json({ 
      success: true, 
      message: `Toplam ${allBlogPosts.length} blog yazısı başarıyla eklendi.`,
    ...
    return res.status(500).json({ 
      success: false, 
      message: 'Blog yazıları eklenirken bir hata oluştu.',
      error: error.message 
    });
  }
}